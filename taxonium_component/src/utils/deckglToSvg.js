      ...
      switch (layer.layerType) {
        ...
        case "ScatterplotLayer":
          for (const point of layer.data) {
            ...
            const accessor = layer.getFillColor ? layer.getFillColor : layer.getColor;
            let color
            if (!accessor) {
            ...
            ${layer.stroked ? `stroke="rgb(${strokeColor})" stroke-width="${strokeWidth}"` : ""} />`;
          }
          break;
        ...
        case "LineLayer":
          for (const line of layer.data) {
            ...
            const colorAccessor = layer.getLineColor ? layer.getLineColor : layer.getColor;
        ...
        case "TextLayer":
          ...
          for (const text of layer.data) {
            ...
            const alignment = accessOrConstant(layer.getAlignmentBaseline, text);
            ...
            const newContent = `<text x="${x}" y="${y}" font-family="${layer.fontFamily}" font-weight="${layer.fontWeight}" fill="rgb(${color})"
            font-size="${size}" text-anchor="${anchor}" alignment-baseline="${alignment}" dx="${pixelOffset[0]}" dy="${pixelOffset[1]}"
