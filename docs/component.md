...
- The directive uses React 18's `createRoot` API for rendering
- Server-side rendering is handled by checking `typeof window !== 'undefined'`
- The component dynamically imports `taxonium-component` to avoid SSR issues
- Inputs are reactive through Angular's `OnChanges` lifecycle hook
