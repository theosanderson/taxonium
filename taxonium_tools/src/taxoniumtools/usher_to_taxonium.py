...
parser.add_argument(
    '--chronumental',
    action='store_true',
    help='If set, we will run chronumental to generate a time tree')

parser.add_argument('--chronumental_steps',
                    type=int,
                    help='Number of steps to use for chronumental',
                    default=500)
...
if args.chronumental:
    ...
    # Check if chronumental is available:
    if not os.system("chronumental -h > /dev/null"):
        # The command returns 0 when chronumental is there, so if not, we can run:

        result = os.system(
            f"chronumental --tree /tmp/distance_tree.nwk --dates {args.metadata} --steps {args.chronumental_steps} --tree_out /tmp/timetree.nwk --dates_out /tmp/date_comparison.tsv.gz"
        )

        if result != 0:
            print("Error running chronumental")
            exit(1)
        ...
        print("####")
        print("CHRONUMENTAL WAS NOT FOUND. A TIME TREE WILL NOT BE BUILT.")
        print("YOU CAN INSTALL CHRONUMENTAL WITH `pip install chronumental`")
        print("####")
